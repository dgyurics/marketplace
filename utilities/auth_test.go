package utilities

import (
	"testing"
	"time"

	"github.com/golang-jwt/jwt/v5"
	"github.com/stretchr/testify/assert"
)

const (
	privateKeyPEM = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`
	publicKeyPEM = `-----BEGIN PUBLIC KEY-----

sQIDAQAB
-----END PUBLIC KEY-----`
)

func TestCreateToken(t *testing.T) {
	jwtUtility := NewJWTUtility([]byte(privateKeyPEM), []byte(publicKeyPEM))

	userID := "user123"
	duration := time.Hour

	tokenString, err := jwtUtility.CreateToken(userID, duration)
	assert.NoError(t, err, "expected no error in creating token")
	assert.NotEmpty(t, tokenString, "expected a non-empty token string")

	// Verify token structure
	token, err := jwt.Parse(tokenString, func(token *jwt.Token) (interface{}, error) {
		return jwt.ParseRSAPublicKeyFromPEM([]byte(publicKeyPEM))
	})
	assert.NoError(t, err, "expected no error in parsing token")
	assert.True(t, token.Valid, "expected token to be valid")
}

func TestVerifyToken(t *testing.T) {
	jwtUtility := NewJWTUtility([]byte(privateKeyPEM), []byte(publicKeyPEM))

	userID := "user123"
	duration := time.Hour

	// First, create a token to verify
	tokenString, err := jwtUtility.CreateToken(userID, duration)
	assert.NoError(t, err, "expected no error in creating token")

	// Now, verify the token
	claims, err := jwtUtility.VerifyToken(tokenString)
	assert.NoError(t, err, "expected no error in verifying token")
	assert.NotNil(t, claims, "expected claims to be non-nil")

	mapClaims := claims.(jwt.MapClaims)
	assert.Equal(t, userID, mapClaims["user_id"], "expected user_id to match")
}

func TestVerifyInvalidToken(t *testing.T) {
	jwtUtility := NewJWTUtility([]byte(privateKeyPEM), []byte(publicKeyPEM))

	// Create an invalid token (e.g., by modifying a valid one)
	invalidTokenString := "invalid.token.string"

	// Attempt to verify the invalid token
	_, err := jwtUtility.VerifyToken(invalidTokenString)
	assert.Error(t, err, "expected an error when verifying an invalid token")
}

func TestCreateRefreshToken(t *testing.T) {
	jwtUtility := NewJWTUtility([]byte(privateKeyPEM), []byte(publicKeyPEM))

	// Generate a refresh token
	refreshToken, err := jwtUtility.CreateRefreshToken()
	assert.NoError(t, err, "expected no error in creating refresh token")
	assert.NotEmpty(t, refreshToken, "expected a non-empty refresh token")

	// Check that the refresh token has a reasonable length (since it's hex-encoded, it should be 64 characters for a 32-byte token)
	assert.Equal(t, 64, len(refreshToken), "expected refresh token to be 64 characters long")
}
